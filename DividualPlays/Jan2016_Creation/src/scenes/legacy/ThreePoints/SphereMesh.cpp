...
//----------------------------------------------------------
static ofMesh ofGetIcosahedronMesh(float radius) {
    ofMesh mesh;
...
//----------------------------------------------------------
static ofMesh ofGetIcoSphereMesh(float radius, int iterations) {
...
static ofMesh _icosphere;

void _ofSetIcoSphereResolution(int iterations) {
    _icosphere = ofGetIcoSphereMesh(1, iterations);
}
...
void ofIcoSphere(ofVec3f center, float radius) {
    ...
    _icosphere.drawVertices();
    _icosphere.drawWireframe();
    ofPopMatrix();
}
