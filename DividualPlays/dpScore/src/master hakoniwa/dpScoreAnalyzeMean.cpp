...
void AnalyzeMean::update()
{
	...
	if (mMaster) {
		if (mTotalAddition.f >= mMeanLimit) {
			...
			const int sandStorm = getMH().getSceneController().getUniqueScenes().getIndex("dpVisSandStorm");
			const int stage = getMH().getSceneController().getUniqueScenes().getIndex("dpVisStage");
			const int tornado = getMH().getSceneController().getUniqueScenes().getIndex("dpVisTornado");
			const int size = getMH().getSceneController().getUniqueScenes().size();
			if (tornado != size && size < 7) {
				getMH().getSceneController().setUniqueScene(tornado, mWin0, mWin1);
				mPrevScene = tornado;
				mPrevSetSceneTime = t;
				ofLogNotice() << "Force loaded dpVisTornado";
			}
			else if (stage != size && size < 6) {
				getMH().getSceneController().setUniqueScene(stage, mWin0, mWin1);
				mPrevScene = stage;
				mPrevSetSceneTime = t;
				ofLogNotice() << "Force loaded dpVisStage";
			}
			else if (sandStorm != size && size < 5) {
				getMH().getSceneController().setUniqueScene(sandStorm, mWin0, mWin1);
				mPrevScene = sandStorm;
				mPrevSetSceneTime = t;
