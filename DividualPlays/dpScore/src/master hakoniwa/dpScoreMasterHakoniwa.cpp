...
void MasterHakoniwa::setupUI(ofxUITabBar* tabbar)
{
	...
	meanTab->addSlider("Min Time", 0.1f, 60.f * 5.f, &mAnalyzeMean.mMinSetSceneTime, w - kMargin, kLineHeight);
...
void MasterHakoniwa::initialize()
{
	...
	xml.pushTag("osc");
	auto host = xml.getAttribute("serverLighting", "host", "127.0.0.1");
	auto port = xml.getAttribute("serverLighting", "port", 10001);
	xml.popTag();
	mOscSenderLighting.setup(host, port);
}
	...
	if (widgetName == "Analyze") {
		...
		if (radio->getActive()->getValue()) {
			if (toggleName == "Maestro") {
				...
				mAnalyzeMean.mMaster = true;
				mAnalyzePixelate.mMaster = false;
				mAnalyzeMean.mMeanLimit = 20.f;
				mAnalyzeMean.mMinSetSceneTime = 60.f * 3.f;
                for (int i=0; i<ofVec4f::DIM; i++) {
                    mAnalyzeMean.mMeanAddtion[i] = mAnalyzeMean.mMeanLimit / ofVec4f::DIM;
                }

				ofxOscMessage m;
				m.setAddress(kOscAddrLighting);
				m.addIntArg(1);
				mOscSenderLighting.sendMessage(m);
			}
