...
void ofxKsmrParallellinkDelta::setup(float Radius, float armLength,float arm2Length,float height,float plotRadius){
	...
	for (int i = 0;i < 3;i++){
		actuator[i].setPosition(cos(ofDegToRad(90-i*120 + rotation))*radius, height,
								sin(ofDegToRad(90-i*120 + rotation))*radius);
...
void ofxKsmrParallellinkDelta::setPlot(ofVec3f pos){
	...
	for (int i = 0;i < 3;i++){
		plots[i].set(cos(ofDegToRad(90-i*120 + rotation))*plot_radius, 0.0,
					 sin(ofDegToRad(90-i*120 + rotation))*plot_radius);
		plots[i] += pos;
